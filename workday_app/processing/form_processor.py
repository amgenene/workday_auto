...
class FormProcessor:
    ...
    def _find_required_fields(self) -> List[Any]:
        ...
        return self.element_handler.find_elements_safely(By.XPATH, "//abbr[text()='*']")
    ...
    def _handle_questions(
        self, questions: List[Tuple[str, Any, str]], step: int
    ) -> bool:
        ...
        for question_text, element, automation_id in questions:
            if not self.question_matcher.process_question(
                question_text, element, automation_id, step
            ):
                success = False
